    ...
    try {
      ...
      audioElements.forEach(audio => {
        ...
        if (audioWithSink.setSinkId && typeof audioWithSink.setSinkId === 'function') {
          // Функция для установки speaker с повторными попытками
          const setSpeakerWithRetry = async (maxRetries = 3) => {
            for (let attempt = 0; attempt < maxRetries; attempt++) {
              try {
                ...
                // Если есть второе устройство, используем его
                if (audioOutputs.length > 1) {
                  await audioWithSink.setSinkId(audioOutputs[1].deviceId);
                } else {
                  // Если устройство только одно, используем пустую строку (default)
                  await audioWithSink.setSinkId('');
                }
                
                // Если успешно установили, выходим
                return;
              } catch (e) {
                // Если это последняя попытка, пробуем пустую строку
